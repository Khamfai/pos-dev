<template>
  <q-page class="q-pa-md">
    <section>
      <div class="tw:grid tw:grid-cols-2 tw:gap-4 tw:md:grid-cols-3 tw:lg:grid-cols-4">
        <div class="tw:rounded" style="height: 100px; background-color: rgba(255, 255, 255, 0.3)">
          Column-1
        </div>
        <div class="tw:rounded" style="height: 100px; background-color: rgba(255, 255, 255, 0.3)">
          Column-2
        </div>
        <div class="tw:rounded" style="height: 100px; background-color: rgba(255, 255, 255, 0.3)">
          Column-3
        </div>
        <div class="tw:rounded" style="height: 100px; background-color: rgba(255, 255, 255, 0.3)">
          Column-4
        </div>
      </div>
    </section>
    <section class="tw:my-4">
      <div class="tw:grid tw:grid-cols-1 tw:gap-4 tw:md:grid-cols-2 tw:lg:grid-cols-2">
        <div class="tw:rounded" style="height: 300px; background-color: rgba(255, 255, 255, 0.3)">
          Column-1
        </div>
        <div class="tw:rounded" style="height: 300px; background-color: rgba(255, 255, 255, 0.3)">
          Column-2
        </div>
      </div>
    </section>
    <section class="tw:my-4">
      <div class="tw:grid tw:grid-cols-1 tw:gap-4 tw:md:grid-cols-2 tw:lg:grid-cols-2">
        <div class="tw:rounded" style="height: 300px; background-color: rgba(255, 255, 255, 0.3)">
          Column-1
        </div>
        <div class="tw:rounded" style="height: 300px; background-color: rgba(255, 255, 255, 0.3)">
          Column-2
        </div>
      </div>
    </section>
  </q-page>
</template>

<script setup lang="ts">
// import { ref } from 'vue';

// const data = ref({
//   series: [
//     {
//       name: 'series1',
//       data: [31, 40, 28, 51, 42, 109, 100],
//     },
//     {
//       name: 'series2',
//       data: [11, 32, 45, 32, 34, 52, 41],
//     },
//   ],
//   chartOptions: {
//     chart: {
//       height: 350,
//       type: 'area',
//     },
//     grid: {
//       show: false,
//     },

//     dataLabels: {
//       enabled: false,
//     },
//     stroke: {
//       curve: 'smooth',
//     },
//     xaxis: {
//       type: 'datetime',
//       categories: [
//         '2018-09-19T00:00:00.000Z',
//         '2018-09-19T01:30:00.000Z',
//         '2018-09-19T02:30:00.000Z',
//         '2018-09-19T03:30:00.000Z',
//         '2018-09-19T04:30:00.000Z',
//         '2018-09-19T05:30:00.000Z',
//         '2018-09-19T06:30:00.000Z',
//       ],
//     },
//     tooltip: {
//       x: {
//         format: 'dd/MM/yy HH:mm',
//       },
//     },
//   },
// });
</script>
